...
# Lazy imports to avoid pydantic version conflicts and circular imports
_DynamicAgent = None
_BaseAgent = None


def _import_dynamic_agent():
    """Lazily import DynamicAgent."""
...
def get_agent(agent_slug: str):
    ...
    Resolves the agent from the database (Agent model) and returns a DynamicAgent.

    Args:
        agent_slug: Agent slug identifier (e.g. 'skis', 'vehicles', 'dj_equipment')

    Returns:
        Instance of DynamicAgent

    Raises:
        ValueError: If no enabled Agent record exists for the given slug
    """
...
def get_agent_by_id(agent_id: int):
    """
    Get a specialist agent by database primary key.

    Args:
        agent_id: The Agent model primary key
...
def __getattr__(name):
    """Lazy attribute access for DynamicAgent and BaseAgent."""
    _import_dynamic_agent()
    mapping = {
